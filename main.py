...
class OcrOfficial:
    def __init__(self, **kwargs):
        ...
        self.model_cccd_fields_detector = torch.hub.load("ultralytics/yolov5", 'custom',
                                                         path=kwargs["wcccdf_path"], trust_repo=True)
        ...
        try:
            ...
            for im_name in im_list:
                ...
                if type == "cavet":
                    ...
                    # If it is cavet
                    if isImageContainCavetInRightDirection(im=cavet_detect_crop_im) or True:
                        ...
                        for i in range(len(list_of_set)):
                            ...
                            # Save image
                            cv2.imwrite(
                                save_im_dir, cavet_fields_detect_crop_im)
                            # Text recognizer
                            try:
                                # read_im = Image.open(
                                #     save_im_dir) if class_i != 'plate' else process(save_im_dir)

                                read_im = processImageBeforeRecognitionText(
                                    save_im_dir)
                    ...
                    # Cavet detector
                    result_cccd_detector = self.model_cavet_detector(
                        im_path, size=640)
                    # Check if result is empty
                    if len(result_cccd_detector.pandas().xyxy[0]) == 0:
                        print(f"Can not detect cccd in {im_name}")
                        # Remove folder
                        shutil.rmtree(os.path.join(
                            self.save_cavet_detector_path, im_name_without_ext))
                        continue
                    ...
                    # If it is cavet
                    if isImageContainCavetInRightDirection(im=cccd_detect_crop_im) or True:
                        # Cavet fields detector
                        result_cavet_fields_detector = self.model_cccd_fields_detector(
                            cccd_detect_crop_im, size=640)
                        # Check if result is empty
                        if len(result_cavet_fields_detector.pandas().xyxy[0]) == 0:
                            continue
                        # Get all Classes, if there is a class that has 2 or more bounding boxes, choose the one with the highest confidence
                        ...
                        for i in range(len(list_of_set)):
                            ...
                            # Save image
                            cv2.imwrite(
                                save_im_dir, cccd_fields_detect_crop_im)
                            # Text recognizer
                            try:
                                # read_im = Image.open(
                                #     save_im_dir) if class_i != 'plate' else process(save_im_dir)
